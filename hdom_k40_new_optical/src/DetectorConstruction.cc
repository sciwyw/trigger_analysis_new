...
G4VPhysicalVolume *DetectorConstruction::DefineVolumes()
{
  ...
  for (int i = 0; i < numOftheta0; i++) {
    //G4cout<<"theta : "<< theta_array0[i]  << G4endl;
    for (int j = 0; j < int(phi_array0[i].size()); j++) {

      //G4cout<<"phi"<< phi_array0[i][j] << G4endl;
  ...
  // G4cout<<"number of theta"<< numOftheta0 << G4endl;
  ...
  //G4cout<<"number of PMTs"<< transform0.size()<< G4endl;
